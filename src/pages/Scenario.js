...
const exampleScenarioData = {
  initialStateId: 'arrival',
  scenes: [
    {
      id: 'arrival',
      mustBeCorrect: false,
...
const ScenarioContainer = () => {
  ...
  useEffect(() => {
    ...
    // Create a promise that resolves in 1.5s to simulate
    // an asynchronous fetch
    new Promise(res => {
      setTimeout(() => res(exampleScenarioData), 1500);
    }).then(data => {
  ...
  return (
    <div className={styles.container}>
...

      {
        ...
        /> : <p>No scene data yet, probably loading...</p>
      }
    </div>
  );
};
